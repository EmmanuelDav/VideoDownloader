...
class PauseReceiver: BroadcastReceiver() {
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (result) {
            ...
            val newInputData = workDataOf("taskId" to taskId, "isPaused" to isPaused)
            val newWorkRequest = OneTimeWorkRequestBuilder<DownloadWorker>()
                .setInputData(newInputData)
                .addTag(taskId)
                .build()
            WorkManager.getInstance(context!!).enqueueUniqueWork(taskId, ExistingWorkPolicy.REPLACE, newWorkRequest)
